 ...
 * OPTIMIZED: Parallel API calls, no delays
 */
async function fetchAllNBAData(season = DEFAULT_SEASON) {
  console.log(`Starting NBA data fetch for ${season}...\n`);

  try {
    // 🚀 OPTIMIZATION: Fetch both API calls in parallel
    // Previous: ~4 seconds (sequential)
    // Now: ~2 seconds (parallel)
    ...
    console.log(`\n✅ Data fetch complete: ${combinedData.length} teams\n`);
    return combinedData;
  } catch (error) {
